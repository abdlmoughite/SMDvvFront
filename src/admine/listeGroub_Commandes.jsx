...
const ListeGroubCommandes = () => {
    ...
    const list_coust_groub = Array.isArray(list_coust_groub_sort) ? 
    list_coust_groub_sort.sort((a, b) => new Date(b.date_coust) - new Date(a.date_coust)) : [];
    ...
    const handleUpdate = (id, newCost) => {
        ...
        dispatch(get_coust_groub());
        // alert('coust mis a jour avec success');
    }
...
return (<>
    ...
    <div className="bg-slate-800 text-white font-bold p-4 rounded-lg shadow-md m-9">
        number de jours {list_coust_groub.length}
        <button onClick={openPopup} className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 ml-4">Ajouter Jour</button>
    </div>
    {isOpen && (
