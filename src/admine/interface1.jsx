...
function Add_commande() {
    ...
    const enregistrer_commande_ozon = async () => {
        for (const i of commandes) {
            try {
                ...
                formData.append("products", JSON.stringify([{ ref: "", qnty: 2 }]));
                ...
                if (res.data["ADD-PARCEL"]?.["RESULT"] =='SUCCESS') {
                    ...
                    toast.success("✅ Commande envoyée avec succès ! "+i.id_commande);
                } else {
                    toast.error("❌ Échec de l'envoi de la commande ! "+i.id_commande);
                    console.error("Erreur lors de l'envoi de la commande :", res.data);
                }
            } catch (error) {
    ...
    const findville = (villeName) => {
        const foundVille = list_villes.find(ville => ville.value === villeName);
        return foundVille
    }
...
const ajouterExcelData_json = () => {
    if (excelData && Array.isArray(excelData)) {
        excelData.forEach((row) => {
            ...
            if (firstNineColumns.every(value => value === "" || value === null)) {
                return; 
            }
            ...
            const newCommande = {
                ...
                ville_id : findville(row["VILLE"]) ? findville(row["VILLE"]).id : "",
                prix_livraison: findville(row["VILLE"]) ? findville(row["VILLE"]).DELIVERED : 0,
                quntite: 1, 
                prix: row["PRIX"] || 0, 
            ...
            dispatch(add_commandes(newCommande));
        });
...
const modifieVille = (idcommande , newville) => {
    const commandeRecherch = commandes.find(item => item.id_commande === idcommande) 
            ...
            {loading === true || user === undefined ? (
                ...
                <div className="w-full m-4">
                    <div className="w-full p-6 bg-gray-800 shadow-lg rounded-lg border border-gray-700">
                        <h1 className="text-xl font-semibold text-blue-400 mb-6">Formulaire de Commande</h1>
                        <form className="space-y-4">
                            <div className="flex gap-3 justify-center w-full">
                            <div className="mr-2">
                                <label className="block text-gray-300 text-sm font-medium mb-1">Nom du client</label>
                                <input type="text" value={commande.nom_client} onChange={get_value} name="nom_client"
                                ...
                                {errors.nom_client && <p className="text-red-400 text-xs mt-1">{errors.nom_client}</p>}
                            </div>

                            <div className="mr-2">
                                <label className="block text-gray-300 text-sm font-medium mb-1">Numéro du client</label>
                                <input type="text" value={commande.numero_client} onChange={get_value} name="numero_client"
                                ...
                                {errors.numero_client && <p className="text-red-400 text-xs mt-1">{errors.numero_client}</p>}
                            </div>
                            
                            <div>
                                <label className="block text-gray-300 text-sm font-medium mb-1">Adresse du client</label>
                                <textarea  value={commande.adresse_client} onChange={get_value} name="adresse_client"
                                    className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-400 focus:outline-none" />
                                {errors.adresse_client && <p className="text-red-400 text-xs mt-1">{errors.adresse_client}</p>}
                            </div>
                            
                            <div>
                                <label className="block text-gray-300 text-sm font-medium mb-1">Produit</label>
                                <select onChange={get_value_produit} name="produit_id"
                                    className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-400 focus:outline-none">
                                    <option value="">Choisir un produit</option>
                                    {list_produit.length > 0 ? list_produit.map(i => (
                                        <option key={i.id_produit} value={i.id_produit} disabled={i.quantite === 0}>
                                            {`${i.nom} - Quantité: ${i.quantite}`}
                                        </option>
                                    )) : <option>Aucun produit disponible</option>}
                                </select>
                                {errors.nom_produit && <p className="text-red-400 text-xs mt-1">{errors.nom_produit}</p>}
                            </div>

                            <div>
                                <label className="block text-gray-300 text-sm font-medium mb-1">Ville</label>
                                <Select options={list_villes} placeholder="Choisir une ville" onChange={get_valueV}
                                    ...
                                    styles={{
                                        ...
                                        option: (base, state) => ({
                                            ...base,
                                            backgroundColor: state.isFocused ? '#93C5FD' : '#4B5563',
                                            color: '#FFFFFF',
                                        }),
                                        singleValue: (base) => ({
                                            ...base,
                            ...
                            <div>
                                <lable className="block text-gray-300 text-sm font-medium mb-1">prix livraison</lable>
                                <input type='number'
                                name="prix_livraison"
                                className="px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-400 focus:outline-none"
                                onChange={get_value}
                            ...
                            <div>
                                <label className="block text-gray-300 text-sm font-medium mb-1">Prix</label>
                                <input type="number" value={commande.prix} onChange={get_value} name="prix"
                                    className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-400 focus:outline-none" />
                                {errors.prix && <p className="text-red-400 text-xs mt-1">{errors.prix}</p>}
                            </div>
                            <div className="flex gap-2">
                            <div>
                                <label className="block text-gray-300 text-sm font-medium mb-1">Commentaire</label>
                                <input type="text" value={commande.commantaire} onChange={get_value} name="commantaire"
                                    className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-400 focus:outline-none" />
                            </div>

                            <div>
                                <label className="block text-gray-300 text-sm font-medium mb-1">Quantité</label>
                                <input type="number" value={commande.quntite} onChange={get_value} name="quntite"
                                    className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-400 focus:outline-none" />
                            </div>
                            </div>
                            <div className="flex justify-center space-x-4 mt-6">
                                <button type="button" onClick={add_commande}
                                    className="w-full py-2 text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow focus:ring-2 focus:ring-blue-400">
                                    Ajouter
                                </button>
                                {button && (
                                    <button type="button" onClick={upDate_commande}
                    ...
                    <div className="w-full p-6 bg-gray-800 shadow-lg rounded-lg border border-gray-700">
                        ...
                        <div className="mb-4">
                            <div className="flex justify-between items-center bg-gray-700 p-4 rounded-lg shadow-sm">
                                <h3 className="text-lg font-medium text-blue-400">
                                    Nombre de commandes: <span className="font-bold">{commandes.length}</span>
                                </h3>
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-600 bg-gray-800 rounded-lg shadow-md">
                                ...
                                <tbody className="bg-gray-800 divide-y divide-gray-600">
                                    {[...commandes].reverse().map((commande, index) => (
                                        ...
                                        >
                                            ...
                                            <td className={`px-6 ${commande.nom_client =='' || commande.nom_client == null  ? 'bg-red-500' : ''} py-4 whitespace-nowrap text-sm text-gray-300`}>
                                                {commande.nom_client}
                                            </td>
                                            <td className={`px-6 ${commande.numero_client =='' || commande.numero_client == null  ? 'bg-red-500' : ''} py-4 whitespace-nowrap text-sm text-gray-300`}>
                                                {commande.numero_client}
                                            </td>
                                            <td className={`px-6 ${commande.ville == '' || commande.ville == null ? 'bg-red-500' : ''} py-4 whitespace-nowrap text-sm text-gray-300`}>
                                                {commande.ville == '?'?
                                                    <Select options={list_villes}
                                            ...
                                            <td className={`px-6 ${commande.nom_client =='' || commande.nom_client == null  ? 'bg-red-500' : ''} py-4 whitespace-nowrap text-sm text-gray-300`}>
                                                {commande.nom_produit == null ? 
                                                        <select  name="produit_id" 
                                                        onChange={(e)=>modifieProduit(commande.id_commande , e.target.value , list_produit[0].nom)}
                                        className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-400 focus:outline-none">
                                        <option value="">Choisir un produit</option>
                                        {list_produit.length > 0 ? list_produit.map(i => (
                                            <option key={i.id_produit} value={i.id_produit} disabled={i.quantite === 0}>
                                                ...
                                                : 
                                            <select name="produit_id" value={commande.produit_id}
                                                        onChange={(e)=>modifieProduit(commande.id_commande , e.target.value , list_produit[0].nom)}
                                    className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-400 focus:outline-none">
                                    <option value="">Choisir un produit</option>
                                    {list_produit.length > 0 ? list_produit.map(i => (
                                        <option key={i.id_produit} value={i.id_produit} disabled={i.quantite === 0}>
                                            ...
                                            <td className={`px-6 ${commande.adresse_client =='' || commande.adresse_client == null  ? 'bg-red-500' : ''} py-4 whitespace-nowrap text-sm text-gray-300`}>
                                                {commande.adresse_client}
                                            </td>
                                            <td className={`px-6 ${commande.nom_client =='' || commande.nom_client == null  ? 'bg-red-500' : ''} py-4 whitespace-nowrap text-sm text-gray-300`}>
                                                {commande.commantaire}
                        ...
                        <div className="mt-4 flex space-x-4">
                            ...
                            />
                            <button onClick={ajouterExcelData_json}>ajouter</button>
